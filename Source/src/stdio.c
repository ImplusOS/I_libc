...
static FILE __stdin = { 0, 0, 0, 1, 0, 0, 0, _IONBF, 0, 0, NULL, {0} };
static FILE __stdout = { 1, 0, 0, 1, 0, 0, 0, _IOLBF, 0, 0, NULL, {0} };
static FILE __stderr = { 2, 0, 0, 1, 0, 0, 0, _IONBF, 0, 0, NULL, {0} };
...
FILE* fopen(const char* path, const char* mode)
{
    ...
    stream->has_unget = 0;
    stream->unget_char = 0;
    stream->buf_mode = (mode[0] == 'r') ? _IOFBF : _IOLBF;
    stream->buf_pos = 0;
    stream->buf_len = 0;
    stream->buf_ptr = stream->buf_internal;
...
size_t fwrite(const void* ptr, size_t size, size_t nmemb, FILE* stream)
{
    ...
    size_t written = 0;
    while (written < total) {
        if (stream->buf_mode != _IONBF && stream->buf_ptr) {
            size_t room = 4096 - stream->buf_pos;
            if (room == 0) {
                if (fflush(stream) != 0) break;
                room = 4096;
            }
            size_t copy = total - written;
            if (copy > room) copy = room;
            memcpy(stream->buf_ptr + stream->buf_pos, src + written, copy);
            stream->buf_pos += copy;
            written += copy;
            if (stream->buf_mode == _IOLBF) {
                for (size_t i = 0; i < copy; i++) {
                    if (src[written - copy + i] == '\n') {
                        if (fflush(stream) != 0) goto fail;
                        break;
                    }
                }
            }
        } else {
            ssize_t n = stdio_write_fd(stream->fd, src + written, total - written);
            if (n <= 0) goto fail;
            written += (size_t)n;
        }
    }
...
int fflush(FILE* stream)
{
    ...
    if (stream->buf_mode != _IONBF && stream->buf_ptr && stream->buf_pos > 0) {
        ssize_t n = stdio_write_fd(stream->fd, stream->buf_ptr, stream->buf_pos);
        if (n < 0) {
            stream->error = 1;
            return EOF;
        }
...
int setvbuf(FILE* stream, char* buf, int mode, size_t size)
{
    ...
    if (mode == _IONBF) {
        ...
        stream->buf_mode = (mode == _IOLBF) ? _IOLBF : _IOFBF;
        stream->buf_ptr = buf;
        (void)size;
    } else {
        stream->buf_mode = (mode == _IOLBF) ? _IOLBF : _IOFBF;
...
int fgetc(FILE* stream)
{
    ...
    if (stream->buf_mode != _IONBF && stream->buf_ptr) {
        if (stream->buf_pos >= stream->buf_len) {
            ssize_t n = file_read(stream->fd, stream->buf_ptr, 4096);
            if (n <= 0) {
                if (n == 0) stream->eof = 1;
                else stream->error = 1;
                return EOF;
        ...
        return (unsigned char)stream->buf_ptr[stream->buf_pos++];
    }
    unsigned char c;
...
int fputc(int c, FILE* stream)
{
    ...
    if (stream->buf_mode != _IONBF && stream->buf_ptr) {
        ...
        stream->buf_ptr[stream->buf_pos++] = ch;
...
int fputs(const char* s, FILE* stream)
{
    ...
    while (written < len) {
        if (stream->buf_mode != _IONBF && stream->buf_ptr) {
            ...
            size_t copy = len - written;
            if (copy > room) copy = room;
            memcpy(stream->buf_ptr + stream->buf_pos, s + written, copy);
            stream->buf_pos += copy;
            written += copy;
            if (stream->buf_mode == _IOLBF) {
                for (size_t i = 0; i < copy; i++) {
            ...
            ssize_t n = stdio_write_fd(stream->fd, s + written, len - written);
            if (n < 0) {
                stream->error = 1;
                return EOF;
            }
